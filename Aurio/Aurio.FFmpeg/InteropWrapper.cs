...
namespace Aurio.FFmpeg
{
    ...
    internal class InteropWrapper
    {
        ...
        public delegate int CallbackDelegateReadPacket(
            IntPtr opaque,
            IntPtr buffer,
            int bufferSize
        );
        ...
        [UnmanagedFunctionPointer(CC)]
        public delegate long CallbackDelegateSeek(
            IntPtr opaque,
            [MarshalAs(UnmanagedType.I8)] long offset,
            int whence
        );

        public delegate IntPtr d_stream_open_file(Type mode, string filename);
        public delegate IntPtr d_stream_open_bufferedio(
            Type mode,
            IntPtr opaque,
            CallbackDelegateReadPacket readPacket,
            CallbackDelegateSeek seek,
            string filename
        );
        public delegate IntPtr d_stream_get_output_config(IntPtr instance, Type type);
        public delegate int d_stream_read_frame(
            IntPtr instance,
            out long timestamp,
            byte[] output_buffer,
